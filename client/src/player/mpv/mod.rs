...
use crate::core::player::{LoadVideo, MediaPlayerEvent, MediaPlayerTrait, PlayerVideo};
...
impl MediaPlayerTrait for Mpv {
    fn start(&mut self) {
        ...
        if self.status.file.is_some() {
            crate::log!(self.set_property(MpvProperty::Pause, false.into()))
        }
    }
