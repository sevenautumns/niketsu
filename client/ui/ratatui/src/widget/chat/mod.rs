...
impl ThemedWidget for ChatWidgetState {
            ...
            .map(|t| {
                ...
                let head_line = match &t.source {
                    UserMessage(user_name) => {
                        let name = match state.user.name.eq(user_name) {
                            true => Span::styled(user_name, state.theme.base().green()),
                    ...
                    UserAction(user_name) => {
                        ...
                        Line::styled(message, state.theme.accent())
                    }
                    Server => {
                        let message = format!("Server notification{head_message}");
                        Line::styled(message, state.theme.base().light_red())
                    }
                    Internal => {
                        let message = format!("Internal notification{head_message}");
                        Line::styled(message, state.theme.base().red())
                    }
                };
